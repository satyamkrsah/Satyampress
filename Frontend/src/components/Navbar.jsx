...
import { ShoppingCart, Menu, X, Search, Heart, User, Moon, Sun, LogOut } from 'lucide-react';
      ...
      <nav className="glass sticky top-0 z-50 transition-colors duration-300">
        <div className="w-full px-4 sm:px-8 lg:px-12">
          <div className="flex items-center justify-between h-16 md:h-20">
            {/* Logo - Left Side */}
            <Link to="/" className="flex flex-col items-start z-10 shrink-0">
              <span className="font-serif text-xl md:text-2xl font-bold text-black dark:text-cream-dark tracking-wider">
                SATYAM
              </span>
              <span className="text-[9px] md:text-[10px] font-semibold tracking-[0.4em] text-gold uppercase mt-0.5">
                Printing Press
              </span>
            </Link>
            ...
            <div className="flex items-center gap-1 md:gap-3 z-10">
              ...
              <button className="p-2 text-black dark:text-cream-dark hover:text-gold transition-colors hidden sm:block" aria-label="Wishlist">
                <Heart className="h-5 w-5" />
              </button>
              
              {isAuthenticated ? (
                <button onClick={logout} className="p-2 text-black dark:text-cream-dark hover:text-gold transition-colors hidden sm:block flex items-center" aria-label="Logout" title={`Logged in as ${user?.name}`}>
                  <LogOut className="h-5 w-5" />
                </button>
              ) : (
                <Link to="/login" className="p-2 text-black dark:text-cream-dark hover:text-gold transition-colors hidden sm:block" aria-label="Login">
                  <User className="h-5 w-5" />
                </Link>
              )}

              <Link to="/cart" className="relative p-2 text-black dark:text-cream-dark hover:text-gold transition-colors">
          ...
          {isSearchOpen && (
            <div className="pb-4 animate-fade-in">
              <form onSubmit={handleSearchSubmit} className="relative max-w-lg mx-auto">
                ...
                />
                <button type="submit" className="absolute right-3 top-1/2 -translate-y-1/2 text-black dark:text-white hover:text-gold">
          ...
          <div className="md:hidden bg-white dark:bg-background-dark border-t border-black dark:border-white dark:border-black dark:border-white animate-fade-in transition-colors duration-300">
            <div className="px-4 py-4 space-y-1">
              ...
              <div className="border-t border-black dark:border-white dark:border-black dark:border-white pt-3 mt-3">
                <p className="px-3 py-2 text-xs uppercase tracking-wider text-black dark:text-white dark:text-black dark:text-white">Categories</p>
