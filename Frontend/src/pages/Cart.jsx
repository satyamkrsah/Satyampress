...
const Cart = () => {
  const { cartItems, removeFromCart, updateQuantity, cartSummary, loading } =
    useCart();
    ...
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center bg-white dark:bg-background-dark transition-colors duration-300 py-10 px-4 text-center">
        <div className="bg-white dark:bg-background-dark transition-colors duration-300 p-10 md:p-14 max-w-lg w-full border border-black dark:border-white dark:border-black dark:border-white transition-colors duration-300">
          ...
          <h2 className="text-2xl font-serif font-semibold mb-3 text-black dark:text-cream-dark">
            Your cart is empty
          </h2>
          <p className="text-black dark:text-white mb-8 text-sm">
            Browse our collections and add premium printing products to your
            cart.
          </p>
          <Link
            to="/products"
  ...
  console.log("Images =>", product.images);
                    ...
                    >
                      <div className="col-span-1 sm:col-span-6 flex gap-4">
                        ...
                        <div className="flex flex-col justify-center">
                          <span className="text-xs text-gold uppercase tracking-wider mb-1">
                            {product.category?.name || product.category}
                          </span>
                          <Link
                            to={`/product/${product._id}`}
                            className="font-serif text-black dark:text-cream-dark hover:text-gold transition-colors line-clamp-2"
                          >
                            {product.name}
                          </Link>
                          {item.customizations &&
          ...
          <div className="w-full lg:w-96 shrink-0">
            <div className="bg-white dark:bg-background-dark transition-colors duration-300 p-6 md:p-8 border border-black dark:border-white dark:border-black dark:border-white transition-colors duration-300 sticky top-36">
              <h2 className="font-serif text-xl font-semibold mb-6 text-black dark:text-cream-dark">
                Order Summary
              </h2>
              ...
              <div className="space-y-4 mb-6 text-sm">
                ...
                <div className="flex justify-between text-black dark:text-white">
                  ...
                  <span className="font-semibold text-black dark:text-cream-dark">
                    ₹
                    {cartSummary.gstAmount.toLocaleString("en-IN", {
              ...
              <p className="text-xs text-black dark:text-white text-center mb-6">
                Free delivery on orders above ₹1,000
              </p>

              <Link
                to="/checkout"
                className="btn-primary w-full flex items-center justify-center gap-2 h-12 mb-4"
              >
