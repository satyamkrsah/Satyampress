...
console.log(payment);
console.log("Payment Model Name:", payment.constructor.modelName);
...
try {
    ...
    console.error("KEY ID:", process.env.RAZORPAY_KEY_ID);
    console.error(
      "KEY SECRET:",
...
exports.verifyPayment = async (req, res, next) => {
  try {
    ...
    if (!order) {
      return res
        .status(404)
        .json({ success: false, error: "Associated order not found" });
    }
  ...
  try {
    ...
    if (payment.paymentStatus !== "completed") {
      return res
        ...
        .json({
          success: false,
          error: "Payment is not completed. Cannot refund.",
        });
    }
    ...
    if (payment.refundStatus === "full") {
      return res
        .status(400)
        .json({ success: false, error: "Payment is already fully refunded." });
    }
    ...
    if (refund) {
      payment.refundStatus =
        amount && amount < payment.amount ? "partial" : "full";
